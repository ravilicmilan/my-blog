...
app.Views.Pagination = Backbone.View.extend({
	...
	initialize: function (options) {
	    ...
	    var rangeMin = (range % 2 == 0) ? (range / 2) - 1 : (range - 1) / 2;  // 3
	    var rangeMax = (range % 2 == 0) ? rangeMin + 1 : rangeMin;  // 3
	    var pageMin = currentLinkNumber - rangeMin;  // 1-3=-2
	    var pageMax = currentLinkNumber + rangeMax;  // 1+3=4

	    pageMin = (pageMin < 1) ? 1 : pageMin; // 1
	    pageMax = (pageMax < (pageMin + range - 1)) ? pageMin + range - 1 : pageMax; // 4

	    if (pageMax > numOfPages) { // jeste 
	        pageMin = (pageMin > 1) ? numOfPages - range + 1 : 1; // 1
    	...
    	/*console.log('numOfPages: ' + this.numOfPages, ' currentLinkNumber: ' + 
    		this.currentLinkNumber, ' pageMin: ' + this.pageMin, ' pageMax: ' + 
    		this.pageMax + ' url: ' + this.pageUrl);
    	console.log(options);*/
	},
